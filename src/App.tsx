...
// Detectar tokens del hash URL según documentación oficial
function handleSSOReturn(): { access_token?: string; refresh_token?: string } {
  ...
  if (access_token || refresh_token) {
    console.log('🔍 Tokens recibidos del Hub SSO:', {
      access_token: access_token ? `${access_token.substring(0, 20)}...` : 'no',
      refresh_token: refresh_token ? `${refresh_token.substring(0, 20)}...` : 'no',
...
function clearHash(){
  // Limpiar URL como recomienda la documentación
...
function App() {
  ...
  useEffect(() => {
    // Ejecutar al cargar la página según documentación oficial
      ...
      try {
        if (access_token) {
          ...
          try {
            ...
            // Guardar tokens según documentación oficial
